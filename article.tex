\documentclass[12pt]{article}
  ...
  \author{Victor H. Fernandes\inst{1}, Marcelo M. Oliveira\inst{1}, Lucas A. Martins \inst{1}}
...
Nascida em meados da década de 2000 \cite{jing2010}, a computação em nuvem vem ganhando espaço
nos congressos e debates da área de tecnologia e se tornando uma ferramenta cada vez mais comum
no dia a dia dos usuários de dispositivos eletrônicos em geral. Esta tecnologia fornece uma grande
quantidade de recursos, aplicações e serviços que atendem usuários que vão desde o desenvolvedor
até o usuário doméstico, com um custo mais baixo \cite{jing2010}, fácil utilização \cite{alizadeh2013} e
maior flexibilidade \cite{kumar2014}.

Paralelamente, a popularidade \textit{smartphones} tem crescido vertiginosamente \cite{atta2013}.
Fornecendo aplicações dos mais diversos gêneros, como jogos, reprodutores de mídia, editores de texto,
redes sociais, entre outros, este dispositivo tem se tornado cada vez mais uma parte indispensável
da vida diária das pessoas.

Infelizmente, a evolução do \textit{hardware}, e da duração da bateria desses dispositivos
não acompanhou a crescente demanda computacional, gerando gargalos que impedem a utilização
de algumas aplicações em \textit{smartphones} \cite{atta2013}.

Entretanto, o fato da computação em nuvem fornecer recursos computacionais sob demanda via
internet, possibilita que o processamento e armazenamento de dados dos dispositivos móveis
sejam transferidos para as plataformas em nuvem, diminuindo o consumo de bateria, melhorando
a performance dos aplicativos e economizando os recursos de armazenamento do aparelho
\cite{atta2013}.

A computação em nuvem em dispositivos móveis pode ser uma solução inteligente, mas implica
em alguns desafios, que serão descritos no restante deste artigo. A seção 2 apresenta um
resumo sobre a computação em nuvem e a computação em nuvem em dispositivos móveis, baseado
em referencial teórico. A seção 3 apresenta a metodologia utilizada para a execução da
revisão sistemática de literatura, utilizada para a elaboração deste trabalho. Na seção 4
os resultados obtidos serão analisados e discutidos, e, por fim, na seção 5, serão apresentadas
...
\subsubsection{Aspectos chave da Computação em Nuvem}
...
\subsubsection{Modelos Arquiteturais da Computação em Nuvem}
...
\subsubsection{Modelos de Implementação da Computação em Nuvem}
